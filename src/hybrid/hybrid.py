...
if __name__ == "__main__":
    ...
    parser.add_argument(
        ...
        type=str,
        default="",
        help="The language of the runfiles to compute hybrid on. Need "
        "to be set if alpha is not given.",
    )
    parser.add_argument(
        "--alpha",
        type=float,
        default=0,
        help="The alpha weight on dense / sparse score. use the "
        "pre-defined (optimal) one if --lang is specified.",
    )
    ...
    with open(args.output, "w") as output_f:
        ...
        if lang := args.lang:
            if lang not in lang2alpha:
                raise ValueError(
                    f"Unrecognized lang, need to be one of {list(lang2alpha.keys())}."
                )
        ...
        for key in tqdm(list(set(runs_1.keys()).union(set(runs_2.keys())))):
            ...
            hybrid_result = []
            min_dense_score = min(dense_hits.values()) if dense_hits else 0
            max_dense_score = max(dense_hits.values()) if dense_hits else 1
            min_sparse_score = min(sparse_hits.values()) if sparse_hits else 0
            max_sparse_score = max(sparse_hits.values()) if sparse_hits else 1
            for doc in set(dense_hits.keys()) | set(sparse_hits.keys()):
                ...
                score = (
                    sparse_score + alpha * dense_score
                    if args.weight_on_dense
                    else alpha * sparse_score + dense_score
                )
